...
def shortest_path(top_a, top_b, m) : 
    ...
    exclude the oxygene from the bfs search
    """
    found = False
    distance = 0
          ...
          for item_1 in list_1 :
              for item in m :
                  if item[0] == item_1 :
                     ...
                     if io == False and 'O' not in item[1] : list_2.append(item[1])
                     else : pass
                  elif item[1] == item_1 :
                     io = (item[1] in list_initial) 
                     if io == False and 'O' not in item[0] : list_2.append(item[0])
                     else : pass
                  else : pass
...
if __name__ == '__main__':
	...
	name_file = path_local + "/database/bmse000094.str"
